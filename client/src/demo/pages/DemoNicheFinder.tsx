...
export function DemoNicheFinder() {
  const [searchTerm, setSearchTerm] = useState('');
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const { nicheResults, filterNicheResults, savedKeywords, addSavedKeyword, addSavedSearch } = useDemoContext();
  ...
  const handleSaveKeyword = (keyword: string) => {
    ...
    addSavedKeyword(keyword);
    addSavedSearch({ keyword });
    toast.success(`"${keyword}" saved to your niches!`);
  };

  const filteredData = nicheResults.filter((niche) =>
    searchTerm
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Try searching for niches like "fitness", "finance", or "cooking".
              </p>
            </div>
          </div>
          <Link to="/">
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
            ...
            <Button type="submit" disabled={isAnalyzing} className="sm:w-auto">
              {isAnalyzing ? (
                <>
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <CardDescription>
                {filteredData.length} niches found. Ranked by potential profitability vs competition.
              </CardDescription>
            </div>
            {filteredData.length > 0 && (
              <Link to="/demo/results">
                      ...
                      >
                        ...
                        <TableCell>
                          <Button
                            ...
                            disabled={savedKeywords.has(niche.keyword)}
